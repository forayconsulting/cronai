...
The first version was CRONAI (February 28, 2025), a quick Flask app. The second was Maistro (March 7, 2025), a proper rebuild with my friend Aaron Bockelie contributing. Same core idea across both: write prompts in plain English, assign each step its own model and its own MCP-connected tools, chain workflows together, set a cron schedule, and let the whole thing run without you. The only agent CLI that could do any of this was Goose, which had launched one month earlier. MCP itself was three months old. The entire foundation I was building on was wet concrete.

I have been using various versions of this stuff since then. It was never a product. It was a utility that let me work faster. In the 13 months since, I built 57 repos of other tools along the same lines: MCP servers, agent prototypes, CLI skills, deployment patterns. CRONAI and Maistro were two of those.

Then, this quarter, every major AI company showed up to the same idea.

OpenAI shipped Codex **Automations** in February 2026. Cursor shipped **Automations** with cron triggers on March 5. Anthropic shipped `/loop` for Claude Code in March, with one outlet headlining it "Claude Code Gets Cron Scheduling to Run as a Background Worker." A startup called **CronBox** launched on Product Hunt with the tagline "Cron for the AI age."
...
The safe move is not to hide. It is to stay in motion. Build utilities, not products. Accumulate capability, not customers. Use the tools harder than anyone else so that when the right moment does come, one with a real moat, real defensibility, a problem that Anthropic or OpenAI cannot solve with a point release, you are the person who has been living in this space long enough to see it clearly.

I built CRONAI on a Friday morning in February 2025. I rebuilt it as Maistro a week later. Thirteen months later, those same features are shipping across every major AI coding tool. I did not miss an opportunity. I spent those 13 months getting better at something more durable than any single product.
